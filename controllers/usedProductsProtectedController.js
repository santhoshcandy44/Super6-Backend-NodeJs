...
exports.getFeedUserPublishedUsedProductListings = async (req, res) => {
    try {
        ...
        const PAGE_SIZE = page_size ? page_size : 20;
        const result = await UsedProductListing.getFeedUserPublishedUsedProductListings(userId, user_id, PAGE_SIZE, queryNextToken)
        if (!result) {
...
exports.getGuestFeedUserPublishedUsedProductListings = async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            const firstError = errors.array()[0];
